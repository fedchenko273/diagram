...
export const getRectPoint = (
  ...
  const { size: { width, height } = {} } = element;

  const horizontalShift = getShift(position, width);
  const verticalShift = getShift(position, height);

  return getShiftedPosition(element, horizontalShift, verticalShift, direction);
};

export const getShiftedPosition = (element, hShift, vShift, direction) => {
  const {
    coords: { x, y },
    size: { width, height }
  } = element;
  ...
  switch (direction) {
    ...
    case "right":
      return { x: x + width, y: y + vShift };

    case "bottom":
      return { x: x + hShift, y: y + height };
